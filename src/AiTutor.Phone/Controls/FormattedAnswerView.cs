...
public sealed partial class FormattedAnswerView : ContentView
{
    ...
    private static string NormalizeNewLines(string text)
    {
        ...
        return NormalizeMathText(normalized);
    }

    private static string NormalizeMathText(string text)
    {
        if (string.IsNullOrWhiteSpace(text))
        {
            return string.Empty;
        ...
        normalized = Regex.Replace(normalized, @"(?<![A-Za-z])(?:text|mathrm)\{([^{}]*)\}", "$1");
        normalized = Regex.Replace(normalized, @"\\+(?:dfrac|tfrac|frac)\{([^{}]+)\}\{([^{}]+)\}", "$1/$2");
        normalized = Regex.Replace(normalized, @"\\+times\b", "×");
        normalized = Regex.Replace(normalized, @"\\+cdot\b", "·");
        normalized = Regex.Replace(normalized, @"\\+div\b", "÷");
        normalized = Regex.Replace(normalized, @"\\+leq?\b", "≤");
        normalized = Regex.Replace(normalized, @"\\+geq?\b", "≥");
        normalized = Regex.Replace(normalized, @"\\+neq\b", "≠");
        normalized = Regex.Replace(normalized, @"\\+approx\b", "≈");
        normalized = Regex.Replace(normalized, @"(?<=\d)\s+times\s+(?=\d)", " × ", RegexOptions.IgnoreCase);
        normalized = Regex.Replace(normalized, @"(?<=\d)\s+div\s+(?=\d)", " ÷ ", RegexOptions.IgnoreCase);
