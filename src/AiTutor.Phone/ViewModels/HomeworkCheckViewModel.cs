...
public sealed class HomeworkCheckViewModel : ViewModelBase
{
        ...
        ILogger<HomeworkCheckViewModel> logger)
    {
        ...
        BackToInputCommand = new AsyncCommand(() => Shell.Current.GoToAsync("..", false));
        OpenHomeworkImageCommand = new AsyncCommand(() => Shell.Current.GoToAsync("homework-check-image", false));
        BackToResultPageCommand = new AsyncCommand(() => Shell.Current.GoToAsync("..", false));
    ...
    public bool IsBlockingLoading
    {
        get => _isBlockingLoading;
        set
        {
            if (SetProperty(ref _isBlockingLoading, value))
            {
                RefreshComputedState();
            }
        }
    ...
    public bool CanUseInputControls => !IsChecking && !IsBlockingLoading;
    public bool CanStartCheck => HasPreview && !IsChecking && !IsBlockingLoading;
    ...
    public string StartCheckButtonText => IsChecking ? "检查中..." : HasCheckResult ? "重新检查" : "开始检查";
    ...
    public ICommand OpenHomeworkImageCommand { get; }
    public ICommand BackToResultPageCommand { get; }
    ...
    private async Task ProcessSelectedFileAsync(FileResult? file)
    {
        ...
        try
        {
            ...
            ClientStageText = "图片已准备";
            AiStatusText = "等待提交";
            LatestStatusMessage = "图片已选择，点击开始检查后会发送给 AI。";
        }
        catch (Exception ex)
        {
    ...
    private async Task StartCheckAsync()
    {
        ...
        try
        {
            ...
            await MainThread.InvokeOnMainThreadAsync(() => Shell.Current.GoToAsync("homework-check-result", false));
        }
    ...
    private void RefreshComputedState()
    {
        ...
        OnPropertyChanged(nameof(CanUseInputControls));
